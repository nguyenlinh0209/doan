...
//        }
    ...
    public static void getFontsFromLocal(Context context) {
        loadFontsFromDirectory(context.getFilesDir());

        File fontsSubdirectory = new File(context.getFilesDir(), FOLDER);
        if (fontsSubdirectory.exists() && fontsSubdirectory.isDirectory()) {
            loadFontsFromDirectory(fontsSubdirectory);
        }
    }

    private static void loadFontsFromDirectory(File directory) {
        ...
        if (files != null) {
            for (File file : files) {
                try {
                    ...
                    Log.d("duckduck", "Failed to load font: " + file.getName(), e);
                }
            }
        }
    }

    public static void addFont(String fontName, Typeface typeFace) {
        ALL_FONTS.add(new RTTypeface(fontName, typeFace));
    }
