...
import com.onegravity.rteditor.api.media.RTEmojiImpl;
...
import com.onegravity.rteditor.spans.ImageEmojiSpan;
...
import com.onegravity.rteditor.utils.BitmapUtils;
...
public class ConverterHtmlToSpanned implements ContentHandler {
    ...
    public void setEmojiUrl(String url) {
        emojiUrl = url;
    }

    public RTSpanned convertHtmlToSpannable(String text) {
        Date start = new Date();
        Log.d("RTSpanned", "start = " + start.toString());
        mSource = text;
        mParser = new Parser();
        try {
        ...
        Log.d("RTSpanned", "end = " + (end.getTime() - start.getTime()) / 1000);
    ...
    private void startImg(Attributes attributes) {
        Context ctx = RTApi.getApplicationContext();
        int len = mResult.length();
        String src = attributes.getValue("", "src");
        String size = attributes.getValue("", "size");
        mResult.append("\uFFFC");
        ImageEmojiSpan imageSpan = new ImageEmojiSpan(ctx, emojiUrl, src, Integer.parseInt(size));
