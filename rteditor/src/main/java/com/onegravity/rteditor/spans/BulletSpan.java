...
public class BulletSpan extends BaseListItemSpan implements LeadingMarginSpan, RTSpan<Boolean>, RTParagraphSpan<Boolean> {
    ...
    private int mLineText = 0;

    public BulletSpan(int gapWidth, boolean isEmpty, boolean isFirst, boolean isLast, int lineText) {
        mGapWidth = gapWidth;
    ...
    public void drawLeadingMargin(Canvas c, Paint p, int x, int dir, int top, int baseline, int bottom,
        ...
        if (!mIgnoreSpan && spanned.getSpanStart(this) == start) {
            ...
            int textColor = determineTextColor(spanned, start, end, p.getColor());
            p.setStyle(Paint.Style.FILL);
            p.setColor(textColor);

            // draw the bullet point
            mTextSize = Math.max(Math.round(determineTextSize(spanned, start, end, p.getTextSize()) / 9f), 4);

            int lineStart = l.getLineStart(mLineText);
            float xAxis = l.getPrimaryHorizontal(lineStart);

            draw(c, p, x, dir, top, bottom, mTextSize, xAxis);
    ...
    private void draw(Canvas c, Paint p, int x, int dir, int top, int bottom, int size, float xAxis) {
        sBulletPath.reset();
        float newX = Math.max(xAxis - size * 4, 0f);
        sBulletPath.addCircle(newX, 0.0f, size, Direction.CW);

        c.save();
        c.translate(x + dir * size + mGapWidth, (top + bottom) / 2.0f);
